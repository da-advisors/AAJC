...
#write_csv(citizenship, "la_citizenship.csv")
...
sr_2020_LA <- sr_2020_LA %>% left_join(citizenship %>% select(GEO_ID_tract = GEOID, citizenship_perc), by = 'GEO_ID_tract')


# turn into factor for plotting 
sr_2020_LA <- sr_2020_LA %>% 
...
sr_2020_LA$citizenship_perc_fctr <- as.factor(sr_2020_LA$citizenship_perc_fctr)
...
ethnicities_bar
ggsave(filename = "../../AAJC Vis/case_studies/los_angeles/sub_ethn_AA_2020.png", 
...
nhpi_citizen_vars <- c("B05003E_004", "B05003E_009", 'B05003E_0015', 'B05003E_020', # native vars
...
english_county <- english %>% filter(NAME == "Los Angeles County, California") %>% drop_na(citizenship_status) %>% 
...
english_state <- english %>% group_by(citizenship_status, english_ability) %>%
  ...
  mutate(GEOID = '06', NAME = 'California') %>%
  select(GEOID, NAME, citizenship_status, english_ability, estimate)
...
write.xlsx(datasets, file = "../../Transformed Data/2020/citizenship_english_ability_LA.xlsx")
...
# ------
datasets <- list("citizenship" = data.frame(citizenship), "english" = data.frame(english))
write.xlsx(datasets, file = "../../Transformed Data/2020/citizenship_english_ability_LA_NHPI.xlsx")
...
# get county and state data 
housing_county <- housing %>% filter(NAME == "Los Angeles County, California") %>% select(GEOID, NAME, variable, estimate)
...
housing_LA_AA <- rbind(housing_county, housing_state, housing_US)
...
housing_LA_NHPI <- rbind(housing_county, housing_state, housing_US)
