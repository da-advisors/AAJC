...
sr_2020_KC <- sr_2020_KC %>% left_join(citizenship %>% select(GEO_ID_tract = GEOID, citizenship_perc), by = 'GEO_ID_tract')


# turn into factor for plotting 
sr_2020_KC <- sr_2020_KC %>% 
...
sr_2020_KC$citizenship_perc_fctr <- as.factor(sr_2020_KC$citizenship_perc_fctr)


# ------
# plot 
# ------
scatter_response_color <- sr_2020_KC %>% filter(RACE == 'A_A') %>%
...
# Save for Alysha
# write.csv(subethnicity_aa_20, "../../Transformed Data/data for viz_alysha/top_eth_kc.csv")
...
ethnicities_bar
ggsave(filename = "../../AAJC Vis/case_studies/king_county_washington/sub_ethn_AA_2020.png", 
...
ggsave(filename = "../../AAJC Vis/case_studies/king_county_washington/sub_ethn_AA_NATIONAL_2020.png", 
...
citizenship_county <- citizenship %>% filter(NAME == 'King County, Washington')

citizenship_state <- citizenship %>% group_by(citizenship_status) %>%
  summarise(estimate = sum(estimate)) %>% mutate(NAME = 'Washington', GEOID = '53') %>%
...
english_county <- english %>% filter(NAME == "King County, Washington") %>% drop_na(citizenship_status) %>% 
...
english_state <- english %>% group_by(citizenship_status, english_ability) %>%
  ...
  mutate(GEOID = '53', NAME = 'Washington') %>%
  select(GEOID, NAME, citizenship_status, english_ability, estimate)
...
write.xlsx(datasets, file = "../../Transformed Data/2020/citizenship_english_ability_King_County_WA.xlsx")
...
# get county and state data 
housing_county <- housing %>% filter(NAME == "King County, Washington") %>% select(GEOID, NAME, variable, estimate)

housing_state <- housing %>% group_by(variable) %>% summarise(estimate =sum(estimate)) %>% mutate(
  GEOID = '53', NAME = 'Washington'
...
housing_KC_AA <- rbind(housing_county, housing_state, housing_US)


# export data 
datasets <- list("LA - AA" = data.frame(housing_LA_AA), "LA - NHPI" = data.frame(housing_LA_NHPI),
                 "KC - AA" = data.frame(housing_KC_AA))
