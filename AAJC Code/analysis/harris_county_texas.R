...
# graph without grid lines
## AIC
v2_line2 <- agegrp_2010_HC_USA %>% filter(RACE == "A_AIC") %>%
  ggplot(aes(x =as.factor(AGEGRP), y=PERC_DIFF, group = CTYNAME)) +
  geom_hline(yintercept = 0, linetype='dotted', col='grey')+
  geom_line(aes(color=CTYNAME), size=1) +
  scale_color_manual(values = c("#916a92", "#f4c78d"), name = "Region") +
...
ggsave(filename = "../../AAJC Vis/case_studies/harris_county_texas/US_AND_HC_line_graph_coverage_by_agegrp_A_AIC_2010_2.png",
       plot = v2_line2, bg = "white", width =9.07, height = 5.47)

## Asian Alone
v1_line2 <- agegrp_2010_HC_USA %>% filter(RACE == "A_A") %>%
  ggplot(aes(x =as.factor(AGEGRP), y=PERC_DIFF, group = CTYNAME)) +
  geom_hline(yintercept = 0, linetype='dotted', col='grey')+
  geom_line(aes(color=CTYNAME), size=1) +
  scale_color_manual(values = c("#916a92", "#f4c78d"), name = "Region") +
...
ggsave(filename = "../../AAJC Vis/case_studies/harris_county_texas/US_AND_HC_line_graph_coverage_by_agegrp_A_A_2010_2.png",
...
# without grid lines
scatter_response2 <- sr_2020_HC %>% filter(RACE == 'A_A') %>%
  ggplot(aes(x = pop_percentage, y = CRRALL, size = total_tract_pop)) + 
  geom_point(color = "#e49d48", alpha = 0.7) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
ggsave(filename = "../../AAJC Vis/case_studies/harris_county_texas/resp_by_tract_pop_scatter_AA_2020_SIZE_2.png",
...
scatter_response_color2 <- sr_2020_HC %>% filter(RACE == 'A_A') %>%
  ggplot(aes(x = pop_percentage, y = CRRALL, size = total_tract_pop, color = citizenship_perc_fctr)) + 
  geom_point(alpha = .8) + 
  scale_color_brewer(palette = "PuOr") +
  theme(panel.grid.major = element_blank(),
...
ggsave(filename = "../../AAJC Vis/case_studies/harris_county_texas/resp_by_citizenship_A_A_2020_SCATTER_2.png",
...
# Save for Alysha
write.csv(subethnicity_aa_full, "././Transformed Data/data for viz_alysha/case_studies/aa_subethnicities_harriscounty.csv")
