...
# TARGET COUNTY
target_county <- "Utah County"  # replace this val with Salt Lake County
...
scatter_response2 <- sr_2020_UT %>% filter(RACE == 'NHPI_A') %>%
  ggplot(aes(x = pop_percentage, y = CRRALL, size = total_tract_pop)) + 
  geom_point(color = "#e49d48", alpha = 0.7) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(colour = "grey")) +   xlab("NHPI (Alone) Population (%)") + 
  ylab("Cumulative Self-Response\nRate - Overall (%)") + 
  ggtitle("Utah County Response Rate by Percentage\nof NHPI Population by Census Tract - 2020")

ggsave(filename = "../../AAJC Vis/case_studies/utah/resp_by_tract_pop_scatter_NHPI_A_2020_SIZE_utah_2.png",
...
# updated without grid lines
scatter_response_color2 <- sr_2020_UT %>% filter(RACE == 'NHPI_A') %>%
  ggplot(aes(x = pop_percentage, y = CRRALL, size = total_tract_pop, color = citizenship_perc_fctr)) + 
  geom_point(alpha = .8) + 
  scale_color_brewer(palette = "PuOr") +
  theme(panel.grid.major = element_blank(),
  ...
  xlab("NHPI (alone) Population (%)") + 
  ylab("Cumulative Self-Response\nRate - Overall (%)") + 
  ggtitle("Utah County Response Rate by Percentage of NHPI Population and Citizenship Status",
          subtitle =  "Census Tract - 2020") + 
  labs(color = "Citizenship of NHPI\n(alone) Population (%)", size = 'Total Tract Population', size=2) + 
  theme(legend.title = element_text(size = 10), axis.title.y=element_text(size=10), axis.title.x=element_text(size=10))
...
ggsave(filename = "../../AAJC Vis/case_studies/utah/resp_by_citizenship_NHPI_A_2020_SCATTER_utah_2.png",
...
ethnicities_bar
ggsave(filename = "../../AAJC Vis/case_studies/utah/sub_ethn_NHPI_2020_utah_county.png", 
...
ethnicities_bar_NATIONAL
ggsave(filename = "../../AAJC Vis/case_studies/utah/sub_ethn_NHPI_NATIONAL_2020.png", 
...
citizenship_county <- citizenship %>% filter(NAME %in% c("Salt Lake County, Utah", "Utah County, Utah"))

citizenship_state <- citizenship %>% group_by(citizenship_status) %>%
  summarise(estimate = sum(estimate)) %>% mutate(NAME = 'Utah', GEOID = '49') %>%
...
english_county <- english %>% filter(NAME %in% c("Salt Lake County, Utah", "Utah County, Utah")) %>% drop_na(citizenship_status) %>% 
...
write.xlsx(datasets, file = "../../Transformed Data/2020/citizenship_english_ability_Utah_NHPI.xlsx")
...
# get county and state data 
housing_county <- housing %>% filter(NAME %in% c("Salt Lake County, Utah", "Utah County, Utah")) %>% 
...
housing_UT_NHPI <- rbind(housing_county, housing_state, housing_US)
...
datasets <- list("LA - AA" = data.frame(housing_LA_AA), "LA - NHPI" = data.frame(housing_LA_NHPI),
                 ...
                 "AK - NHPI" = data.frame(housing_AK_NHPI), "UT - NHPI" = data.frame(housing_UT_NHPI))
